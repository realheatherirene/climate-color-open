/* ==========================================================================
   Pilot results snapshot — frozen copy of quiz/js/results-ui.js (2026-09-24)
   ==========================================================================
   ...
   Changed from the live file, and only these:
     - imports point at the other snapshot modules;
     - pathway/directory links open in a new tab, so following one never
       navigates away from the pilot's feedback questions;
     - "Copy" shares a link to the public quiz, not to the pilot;
     - no "Retake" button — a retake would start a second session from the
       same person, and the live Retake clears the public quiz's saved
       result, which shares browser storage with this page.
   ========================================================================== */

import { identityBlurbs, colorActivities } from './content.js';
import { getBlend } from './blends.js';
import { renderColorWheel } from './color-wheel.js';

// Outbound links open in a new tab so participants never lose the pilot page
// (and its feedback questions) by following a pathway or directory link.
  ...
  return `
    <div class="styleBlock border-${colorClass}${isPrimary ? ' primary-card' : ''}">
      ...
      <div class="action-palette-cta">
        <a href="${pathwayUrl(colorClass)}" class="btn-pill-soft" ${NEW_TAB}>Explore ${colorKey} ideas &rarr;${NEW_TAB_NOTE}</a>
        <a href="${directoryUrl(colorKey)}" class="btn-pill-soft" ${NEW_TAB}>Explore ${colorKey} resources &rarr;${NEW_TAB_NOTE}</a>
      </div>
    </div>`;
}
...
function palettePillHtml(colorKey, colorClass) {
  return `<a href="${pathwayUrl(colorClass)}" class="palette-pill" ${NEW_TAB}
      style="--pill-true: var(--${colorClass}-color, var(--brand-teal));
             --pill-text: var(--${colorClass}-text, var(--brand-teal));">${colorKey}${NEW_TAB_NOTE}</a>`;
}
...
export function renderResultsScreen(primaryKey, secondaryKey, tertiaryKey) {
  ...
  document.getElementById("btnCopyLink").onclick = () => {
    const repoRoot = window.location.pathname.split('/')[1];
    const shareUrl = `${window.location.origin}/${repoRoot}/quiz/?primary=${encodeURIComponent(primaryKey)}&secondary=${encodeURIComponent(secondaryKey)}&tertiary=${encodeURIComponent(tertiaryKey)}`;
